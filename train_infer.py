...
Usage:
  ...
  Infer:   python train_infer.py --mode infer \
...
class Attention(nn.Module):
    """Channel-wise self-attention (O(C^2), resolution-independent)."""
...
class TransformerBlock(nn.Module):
    def __init__(
            self, dim, num_heads, ffn_expansion_factor=2.66, bias=False):
        super().__init__()
...
class PromptBlock(nn.Module):
    def __init__(self, dim, prompt_dim=64, prompt_len=5):
        ...
        self.prompt_param = nn.Parameter(
            torch.randn(1, prompt_len, prompt_dim, 8, 8))
    ...
    def forward(self, x):
        B, C, H, W = x.shape
        emb = x.mean(dim=[-2, -1])
        prompt_w = self.linear_layer(emb).softmax(dim=-1)
...
class PromptIR(nn.Module):
    def __init__(self, inp_channels=3, out_channels=3, dim=48,
        ...
        self.patch_embed = nn.Conv2d(
            inp_channels, dim, 3, padding=1, bias=bias)
        ...
        self.enc2 = nn.Sequential(
            *[TransformerBlock(
                dim * 2, num_heads[1], ffn_expansion_factor, bias)
        ...
        self.enc3 = nn.Sequential(
            *[TransformerBlock(
                dim * 4, num_heads[2], ffn_expansion_factor, bias)
        ...
        self.bottleneck = nn.Sequential(
            *[TransformerBlock(
                dim * 8, num_heads[3], ffn_expansion_factor, bias)
    ...
    def forward(self, x):
        ...
        e1 = self.prompt1(self.enc1(x0))
        d1 = self.down1(e1)
        e2 = self.prompt2(self.enc2(d1))
        d2 = self.down2(e2)
        e3 = self.prompt3(self.enc3(d2))
        d3 = self.down3(e3)
...
class RestorationDataset(Dataset):
    def __init__(self, data_root, patch_size=128, augment=True):
        ...
        for deg_img in sorted((root / "degraded").glob("*.*")):
            ...
            clean_img = (
                root / "clean" / f"{deg_type}_clean-{img_id}{suffix}")
            if clean_img.exists():
                self.pairs.append((deg_img, clean_img, deg_type))

        assert len(self.pairs) > 0, (
            f"No paired images found under {root}")
...
class TestDataset(Dataset):
    def __init__(self, data_root):
        ...
        assert len(self.images) > 0, (
            f"No test images found in {root}")
    ...
    def __getitem__(self, idx):
        ...
        return TF.to_tensor(padded), path.name, h, w
...
def save_curves(train_losses, val_psnrs, out_dir):
    ...
    axes[0].plot(
        epochs, train_losses, 'b-o', markersize=3, label='Train Loss')
    ...
    if val_psnrs:
        ...
        axes[1].axvline(
            x=best_epoch, color='g', linestyle='--',
...
def parse_args():
    ...
    p.add_argument("--rain_weight", type=float, default=1.5,
                   help="Loss weight multiplier for rain images")
    return p.parse_args()
